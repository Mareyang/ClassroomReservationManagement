...
<head>
    ...
    <title>Professor Portal | PUP Biñan</title>
...
<body>
    ...
    <div class="sidebar">
        <div class="brand"><i class="fa fa-chalkboard-teacher"></i> PUP System</div>
    ...
    <script>
        function loadPage(page, element) {
            ...
            if(page === 'Dashboard') {
                content.innerHTML = `<h1>Professor Dashboard</h1>
                <div class="stats-grid">
                    <div class="stat-box"><i class="fa-solid fa-check-circle" style="color:green; font-size:1.5rem"></i><h3>Approved</h3><p><?php echo $countPApproved; ?></p></div>
                    <div class="stat-box"><i class="fa-solid fa-clock" style="color:orange; font-size:1.5rem"></i><h3>Pending</h3><p><?php echo $countPPending; ?></p></div>
                    <div class="stat-box"><i class="fa-solid fa-circle-xmark" style="color:red; font-size:1.5rem"></i><h3>Rejected</h3><p><?php echo $countPRejected; ?></p></div>
                </div>`;
                ...
                content.innerHTML = `<h1>My Reservations</h1><div class="card"><table>
                    ...
                    <tbody>
                    <?php
                        if (mysqli_num_rows($sqlPReservations) > 0) {
                            while ($r = mysqli_fetch_assoc($sqlPReservations)) {
                ...
                content.innerHTML = `<h1>New Reservation (Priority)</h1>
                    ...
                    <div class="card" style="flex: 2;">
                        <form method="POST" action="add.php" required>
                            <div class="form-grid">
                                ...
                                <button type="submit" class="btn-submit" name="submitClassroom" style="background:var(--gold); color:black;">SUBMIT PRIORITY REQUEST</button>
                            </div>
                        </form>
                    </div>
                    <div style="flex: 1;">
                ...
                <div class="card">
                    <div class="form-grid">
                        <div class="form-group"><label>Professor ID</label><input type="text" value="<?php echo htmlspecialchars($profProfile['professorId'] ?? ''); ?>" readonly></div>
                        <div class="form-group"><label>Employee Number</label><input type="text" value="<?php echo htmlspecialchars($profProfile['employeeNumber'] ?? ''); ?>" readonly></div>
                        <div class="form-group"><label>Professor Type</label><input type="text" value="<?php echo htmlspecialchars($profProfile['professorType'] ?? ''); ?>" readonly></div>
                        <div class="form-group"><label>First Name</label><input type="text" value="<?php echo htmlspecialchars($profProfile['firstName'] ?? ''); ?>" readonly></div>
                        <div class="form-group"><label>Middle Name</label><input type="text" value="<?php echo htmlspecialchars($profProfile['middleName'] ?? ''); ?>" readonly></div>
                        <div class="form-group"><label>Last Name</label><input type="text" value="<?php echo htmlspecialchars($profProfile['lastName'] ?? ''); ?>" readonly></div>
                    </div>
                </div>`;
            }
        }
        function logout() { if(confirm("Are you sure you want to log out?")) window.location.href = 'login.php'; }
        loadPage('Dashboard');
    </script>
</body>
</html>